...
minPercent = min(percent)
print("Maksymalny procent %f \nMinimalny procent %f" % (maxPercent, minPercent))
for i in range(len(countries)- 1):
    print(countries[i], percent[i])
    print(countries[i], int(percent[i]))
    print(countries[i], round(percent[i], 2))
    print(countries[i], int(percent[i] * sumOfPoints/100))
